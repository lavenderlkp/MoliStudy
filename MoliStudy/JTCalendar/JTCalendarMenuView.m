...
- (void)commonInit
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(leftScroll) name:@"LeftArrowTapped" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(rightScroll) name:@"RightArrowTapped" object:nil];
}

- (void) leftScroll
{
    //NSLog(@"1");
    [self setContentOffset:CGPointMake(self.frame.size.width, self.frame.size.height) animated:YES];
}

-(void) rightScroll
{
    //NSLog(@"2");
    [self setContentOffset:CGPointMake(self.frame.size.width * 3, self.frame.size.height) animated:YES];
}
